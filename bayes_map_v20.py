...
class HeuristicMethod:
    ...
    def __init__(self, DG, AG, dict_nodes,
                 ...
                 dict_total,debugging_options,lista_constraints,lista_tareas_constrains,BAYES_DISTANCE, BAYES_LATENCY):
        ...
        self.BAYES_DISTANCE = BAYES_DISTANCE
        self.BAYES_LATENCY = BAYES_LATENCY
        ####constrains
    ...
    def latency_resources(self,BAYES_LATENCY,application_graph, hardware_graph):

        if self.s_prints == 'heudebug':
            print("we are in the function that obtains the set of best resources for each task "
                  "according to the computing latency")
        ...
        cuenta = 0
        for task in application_graph.nodes:
            if application_graph.nodes[task]['op'] not in task_list:
                lista_latencias = []
                lista_recursos = []
                for resource in hardware_graph.nodes:
                    if application_graph.nodes[task]['op'] in hardware_graph.nodes[resource]['op']:
                        try:
                            # we verify the parameters to validate that the resource is capable of allocate the task
                            validacion_parametros = self.verification_of_parameters(task, resource)
                            # if the validation of the parameters and degree is valid
                            if all(validacion_parametros):
                                # we count the resource
                                cuenta = cuenta + 1
                                # next we need to find the computing latency of this resource when allocate this
                                # particular task
                                ...
                                try:
                                    # we gather the data of the computing latency equation
                                    name_function = self.dict_info['functions_res'][
                                        self.dict_nodes[resource]['ops'][application_graph.nodes[task]['op']]['clk']]
                                    # print(worse_function)
                                    # if the computing latency is an equation
                                    if isinstance(name_function, str):
                                        # we make the same process as always in order to evaluate the equation
                                        # todo maybe we can put this to a function itself
                                        vector_total_parametros = self.variable_separation(name_function)
                                        vector_parametro = []
                                        for it in vector_total_parametros:
                                            dummy = "".join(it)
                                            if dummy:
                                            ...
                                            if param_formula == 'width':
                                                ...
                                                for pa in self.dict_nodes_a[task]['param']:
                                                    ...
                                                    if param_formula == pa:
                                                        globals()[pa] = self.dict_nodes_a[task]['param'][pa]
                                        # we evaluate the equation
                                        computing_latency_preliminar = eval(name_function)

                                    else:
                                        # because the computing latency is a constant we assign it to the variable
                                        # directly
                                        computing_latency_preliminar = name_function
                                    # if self.s_prints == 'heudebug':
                                    #     print(f"inside the cycle of the sucessors,"
                                    #           f"the computing latency of the sucessor is ",computing_latency_preliminar)
                                except:
                                ...
                                # the best one
                                lista_latencias.append(computing_latency_preliminar)
                                lista_recursos.append(resource)
                                # if self.s_prints == 'heudebug':
                                #     print("test 01")
                                #     print(nodo_hw, elemento)
                                #     print(self.dict_distancias.item((nodo_hw, elemento)))
                ...
                if self.s_prints == 'heudebug':
                    print(f"la lista de latencias {lista_latencias}, la lista de recursos es {lista_recursos}")
                    print(f"la tarea que estamos evaluando es {application_graph.nodes[task]['op']}")
                    print(f"la cantidad de recursos sera {BAYES_LATENCY}")
                for n in range(BAYES_LATENCY):
                    try:
                        valor_minimo = min(lista_latencias)
                        if self.s_prints == 'heudebug':
                            print(f"el valor minimo ahorita es {valor_minimo}")
                        for m in range(len(lista_latencias)):
                            if lista_latencias[m] == valor_minimo:
                                # print("test")
                                lista_recursos_final.append(lista_recursos[m])
                                lista_recursos.remove(lista_recursos[m])
                                break

                        lista_latencias.remove(valor_minimo)
                    except:
                        if self.s_prints == 'heudebug':
                            print("no mas recursos",lista_recursos_final)
            # print(lista_recursos_final)
            dict_latency[application_graph.nodes[task]['op']] = lista_recursos_final
            task_list.append(application_graph.nodes[task]['op'])
            # print(task_list)
            # input("test primer bug")
        # print(dict_latency)
        # input("test")
        return dict_latency
    ...
    def bayes_function(self,primer_grupo,task_AG,nodo_AG,lista_salida_memoria,lista_final, copia_DG, bandera_recomputo):
        sucesores_nodo_AG = list(self.AG_copy.successors(nodo_AG))
        hypothesis = 1/len(primer_grupo) #####probability that a task will be allocated in a resource ti -> rj
        probability_list = [0 for g in range(len(primer_grupo))]
        ####### we are going to obtain the probability of the evidence given the hipothesis
        for place_of_candidate in range (0,len(primer_grupo)):
            candidate = primer_grupo[place_of_candidate]
            if task_AG in self.DG_original.nodes[candidate]['op']:
                bandera_source_of_data, info_sensor = self.verification_of_source(nodo_AG, candidate)

                bandera_mapping_valido = self.verificacion_data_dependence(candidate,
                                                                           nodo_AG,
                                                                           lista_salida_memoria,
                                                                           lista_final, copia_DG,
                                                                           bandera_recomputo)

                vector_validacion_parametros = self.verification_of_parameters(nodo_AG, candidate)
                if self.s_prints == 'heudebug':
                    print(f"estamos por checar si podemos utilizar este candidato "
                          f"{vector_validacion_parametros}, {bandera_mapping_valido}, "
                          f"{bandera_source_of_data}, {info_sensor}")
                if all(vector_validacion_parametros) and bandera_mapping_valido and bandera_source_of_data:
                    lista_total_nodos = obtencion_sucesores_de_una_lista(self.DG_original, [candidate])
                    if self.s_prints == 'heudebug':
                        print(f"la lista de descendientes del candidato {candidate} es {lista_total_nodos} y los sucesores son {sucesores_nodo_AG}")
                    ...
                    if sucesores_nodo_AG:
                        ...
                        contador_latencia = 0
                        for sucesor_app in sucesores_nodo_AG:
                            tarea_sucesor = self.AG_copia.nodes[sucesor_app]['op']
                            if self.s_prints == 'heudebug':
                                print(f"la tarea es {tarea_sucesor}, lista de nodos {lista_total_nodos} y los nodos son {self.DG_original.nodes}")
                            for sucesor_hw in lista_total_nodos:
                                if tarea_sucesor in self.DG_original.nodes[sucesor_hw]['op']:
                                    contador_tarea = contador_tarea + 1
                                    #######################################################
                                    #########todo aqui podemos integrar mas verificacion
                                    ######################################
                                    if sucesor_hw in self.dict_latency[tarea_sucesor]:
                                        contador_latencia = contador_latencia + 1
                                    paths = list(nx.all_simple_paths(self.DG_original, source=candidate,
                                                                target=sucesor_hw, cutoff=self.BAYES_DISTANCE))
                                    if paths:
                                        contador_distancia = contador_distancia + 1
                            try:
                                distancia = contador_distancia / len(lista_total_nodos)
                                latencia = contador_latencia   / len(lista_total_nodos)
                                tarea = contador_tarea         / len(lista_total_nodos)
                            except:
                                distancia = contador_distancia / 1
                                latencia = contador_latencia / 1
                                tarea = contador_tarea / 1
                            # distancia = contador_distancia / len(lista_total_nodos)
                            # latencia = contador_latencia / len(lista_total_nodos)
                            # tarea = contador_tarea / len(lista_total_nodos)

                            if self.s_prints == 'heudebug':
                                print(f"distancia {distancia}, latencia {latencia}, tarea {tarea}")
                            suma_probabilidad = suma_probabilidad + distancia + latencia + tarea

                        probability_list[place_of_candidate] = suma_probabilidad

                    else:
                        probability_list[place_of_candidate] = 0.99

                else:
                    probability_list[place_of_candidate] = 0
            else:
                probability_list[place_of_candidate] = 0


        suma_total = sum(probability_list)
        if self.s_prints == 'heudebug':
            print(f"the probability list is {probability_list}")
        probability_final = [0 for g in range(len(primer_grupo))]
        for i in range(len(primer_grupo)):
            # print(probability_list[i],suma_total,hypothesis)
            if suma_total != 0:
                buffer_01 = probability_list[i]/suma_total
                buffer_02 = buffer_01*probability_list[i]
                probability_final[i] = buffer_02/hypothesis
            else:
                probability_final[i] = 0
        if self.s_prints == 'heudebug':
            print(f" la tarea que se verifico es {nodo_AG}, sus sucesores son {sucesores_nodo_AG}")
            print("the resulting list is ",probability_final, primer_grupo)
            # input("verificacion")
    ...
    def Mapping(self):
        ...
        self.dict_latency = self.latency_resources(self.BAYES_LATENCY, self.AG_copy, self.DG_copy)
        if self.s_prints == 'heudebug':
            print(self.dict_latency)
        # input("test")
            ...
            if name_task in self.lista_tareas_constrains:
                ...
                while mapping:
                    ...
                    if bandera_primer_nodo:
                        ...
                        lista_probabilidad = self.bayes_function(primer_grupo,task_AG,nodo_AG,lista_salida_memoria,
                                                                 lista_final, copia_DG, bandera_recomputo)
                    ...
                    #### main cycle
                                ...
                                if all(vector_validacion_parametros) and bandera_source_of_data:
                                    if self.s_prints == 'heudebug':
                                        print(f"for the resource {self.dict_nodes[primer_grupo[elemento]]['name']} the "
                                              f"validation is ok")
                                    lista_resultante.append(primer_grupo[elemento])
                                else:
                                    if self.s_prints == 'heudebug':
                        ...
                        if len(lista_resultante) == 1:
                            lista_probabilidad = [0.99]
                            primer_grupo = lista_resultante
                        elif len(lista_resultante) > 1:
                            primer_grupo = lista_resultante
                            lista_probabilidad = self.bayes_function(primer_grupo, task_AG, nodo_AG,
                                                                     lista_salida_memoria,
                                                                     lista_final, copia_DG, bandera_recomputo)

                        else:
                            lista_probabilidad = [0]
                        # if self.s_prints == 'heudebug':
                        #     print(f"el primer grupo es {primer_grupo}")
                        #     if nodo_AG == 1:
                        #         input("test")
                        # if self.s_prints == 'heudebug':
                        ...
                        if max_value == 0 :
                            ...
                            try:
                                ...
                                max_value_error = 0
                            # print(lista_probabilidad_error)
                            # max_value_error = max(lista_probabilidad_error)
                            if max_value_error == 0 and bandera_reinicio:
                                ...
                                for candidato in primer_grupo_buffer:
                                    if lista_salida_memoria[candidato][0] and lista_salida_memoria[candidato][
                                        2] != 'copy':
                                        pass
                                    else:
    ...
    def verificacion_data_dependence(self, recurso_elegido, nodo_AG, lista_salida_memoria, lista_final, copia_DG,
        ...
        if self.s_prints == 'debug' or self.s_prints == 'heudebug':
            print("inside the verifaction data dependence function")
            ...
            for predecesor in set_predecesores:
                ...
                if not bandera_no_esta_en_el_mismo_time_slot and not bandera_recomputo:
                    ...
                    if bandera_recomputo:
                        ...
                        if self.s_prints == 'debug' or self.s_prints == 'heudebug':
                            # if self.dict_nodes_a[nodo_AG]['name'] == 't3': time.sleep(5)
                            print("the elements that we are going to check ", lugar_nodo, recurso_elegido,self.list_sinks_connected_to_rc)
                            ...
                            while done:
                                if copy_list_sinks_connected_to_rc:
                                    ...
                                    sink_nodo_sink_task = sink_node_from_any_node(self.DG_original,
                                                                                  copy_list_sinks_connected_to_rc,
                                                                                  lugar_nodo)
                                # print(sink_nodo_sink_task,lugar_nodo)
                                if time_slot_buffer[sink_nodo_sink_task][0]:
                                    ...
                                    if counter_internal == 5:
                                        ...
                                        sink_nodo_sink_task = sink_node_from_any_node(self.DG_original,
                                                                                      copy_list_sinks_connected_to_rc,
                                                                                      lugar_nodo)
                                        # print("ultima oportunidad ", sink_nodo_sink_task)
                                        done = False
                                        break
                                else:
                                    done = False
